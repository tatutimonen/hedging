...
class Hedger:
    ...
    class HedgingStats:
        ...
        def __repr__(self):
            return (f"[Hedger.{type(self).__name__}]: Using data from sheet {self.sheet_name!r}, performing "
                    ...
                    f"(strikes of {', '.join(map(lambda x: '$' + x, self.strikes))}) with a schedule of {self.schedule} days,"
    ...
    def delta_vega_hedge(data, sheet_name="", portfolio_size=2, schedule=2, cost_basis=0.01):
        ...
        rep_option_BSM = BSM(day0_2.S, int(E_rep), day0_2.r, day0_2.T_norm, day0_2[E_rep])
        ...
        state_prev = Hedger.DeltaVegaState(portfolio, alpha * day0.S, eta * day0_2[E_rep], alpha, eta)
        stats = Hedger.HedgingStats(sheet_name, "delta-vega", portfolio_size, schedule, cost_basis, portfolio_strikes.index)
        stats.total_cost += cost_basis * state_prev.alpha * day0.S + cost_basis * state_prev.eta * day0_2[E_rep]
        ...
        for t, row in day1_onwards.items():
            ...
            rep_option_BSM = BSM.make_from_dict(row2, E_rep)
            portfolio = sum(map(lambda x: x.C_obs, portfolio_BSMs.values()))
            state = Hedger.DeltaVegaState(
                portfolio, state_prev.alpha * row["S"], state_prev.eta * row2[E_rep], state_prev.alpha, state_prev.eta)
            ...
            if t % schedule == 0:
                ...
                state.rep_option = state.eta * row2[E_rep]
                stats.total_cost += abs(
                    cost_basis * (state_prev.alpha - state.alpha) * row["S"] \
                    + cost_basis * (state_prev.eta - state.eta) * row2[E_rep])
